...
export default function MarketFeedPage() {
    ...
    return (
        <div className="container mx-auto px-4 lg:px-8 py-8 space-y-12 pb-24">
            {/* Header */}
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/5 pb-8">
                <div>
                    <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-white mb-2">Market & Strategy Intelligence</h1>
            ...
            {(!strategyData || !strategyData.metrics) && !loading && (
                <div className="p-12 rounded-3xl bg-white/5 border border-white/10 text-center">
                    ...
                    <h3 className="text-xl font-bold text-white mb-2">No active strategies found</h3>
                    <p className="text-slate-400 mb-6">The strategy map is currently being updated by the administration.</p>
                    
                    {session?.user?.role === "ADMIN" && (
                        <button 
                            onClick={handleSeed}
                            className="px-6 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-bold transition-colors"
                        >
            ...
            <div className="pt-12 border-t border-white/5">
                <div className="mb-8">
                    <h2 className="text-2xl font-bold text-white mb-2">Broadcasts & Intelligence</h2>
                    <p className="text-slate-400 text-sm">Latest technical updates, macro analysis, and CEO's vision.</p>
                </div>
                <ContentFeed />
            </div>
        </div>
