...
export default function ProfitDistributionForm({ performancePeriods = [] }: ProfitDistributionFormProps) {
    ...
    return (
        <div className="bg-[#0F172A] rounded-2xl border border-slate-800 shadow-xl p-6 relative overflow-hidden">
            <div className="absolute top-0 right-0 p-4 opacity-5">
                <TrendingUp className="w-32 h-32" />
            </div>

            <h3 className="text-lg font-bold mb-1 text-white dark:text-white">Profit Engine</h3>
            <p className="text-sm text-slate-300 dark:text-slate-300 mb-6">
            ...
            <div className="space-y-4">
                ...
                {availablePeriods.length > 0 ? (
                    <div>
                        <label className="text-xs font-semibold uppercase text-slate-300 dark:text-slate-300 mb-1 flex items-center gap-1">
                            <LinkIcon className="w-3 h-3" /> Link to Performance Period
                        </label>
                        <select
                            value={selectedPeriodId}
                            onChange={handlePeriodChange}
                            className="w-full px-4 py-2 bg-slate-900 border border-white/10 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-sm text-white appearance-none cursor-pointer"
                        >
                            <option value="" className="bg-[#0B1120]">-- Select Ad-hoc / Manual --</option>
                            {availablePeriods.map((p) => (
                                <option key={p._id} value={p._id} className="bg-[#0B1120]">
                ...
                <div>
                    ...
                    <div className="relative">
                        <input
                            ...
                            className={`w-full px-4 py-2 border rounded-lg outline-none transition-colors text-white ${selectedPeriodId
                                ? "bg-slate-800 border-white/5 text-slate-300 cursor-not-allowed"
                                : "bg-slate-900 border-white/10 focus:ring-2 focus:ring-emerald-500"
                                }`}
                        />
                        {selectedPeriodId && <Lock className="w-4 h-4 text-slate-400 absolute right-3 top-2.5" />}
                    </div>
                </div>
                ...
                <div className="flex items-start gap-2 p-3 bg-amber-500/10 dark:bg-amber-500/10 rounded-lg border border-amber-500/20">
